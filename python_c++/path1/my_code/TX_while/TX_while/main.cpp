...
void JumpMan(int x, int y, int sizeX, int sizeY, double jump, COLORREF color, COLORREF bkColor, int jumps, int delay) {
	...
	int i = 0;
	while (i < jumps) {
		DrawMan(x, y - (int)(i % 2 * jump * 10), sizeX, sizeY, color, (i % 2 - 0.5) * jump / 3, 0, (i % 2 - 0.5) * -jump / 3, 0);

		txSleep(delay);

		DrawMan(x, y - (int)(i % 2 * jump * 10), sizeX, sizeY, bkColor, (i % 2 - 0.5) * jump / 3, 0, (i % 2 - 0.5) * -jump / 3, 0);
		i++;
	}

	DrawMan(x, y - (int)(jumps % 2 * jump * 10), sizeX, sizeY, color, (jumps % 2 - 0.5) * -jump / 3, 0);
...
void DrawMan(int x, int y, int sizeX, int sizeY, COLORREF color, double hand, double legs, double head, double twist) {
	txSetColor(color);
...
void AppearEarth(int x, int y, int sizeX, int sizeY, COLORREF from, COLORREF to, int time, int steps) {
	...
	int g0 = txExtractColor(from, TX_GREEN), g1 = txExtractColor(to, TX_GREEN);
	int b0 = txExtractColor(from, TX_BLUE), b1 = txExtractColor(to, TX_BLUE);
